...
{
	static HWND hCkeckBox1;
    switch (message)
    {
	case WM_CREATE:
	{
		hCkeckBox1 = CreateWindow(L"button", L"체크하세요.",
    ...
    case WM_COMMAND:
        {
			...
			case 3001:
			{
				if (SendMessage(hCkeckBox1, BM_GETCHECK, 0, 0) == BST_UNCHECKED) {
					SendMessage(hCkeckBox1, BM_SETCHECK, BST_CHECKED, 0);
				}
				else {
					SendMessage(hCkeckBox1, BM_SETCHECK, BST_UNCHECKED, 0);
				}
			}
				break;
